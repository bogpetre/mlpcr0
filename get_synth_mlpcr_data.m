% function [X, Y, ll, bb_to_X, bb, w] = get_synth_mlpcr_data(n,X_dims,w_df,latent_df,nsrY,nsrX,covPat,...)
%
...
%                   Options are 'isotropic','diagonal', 'full' or a custom
    ...
    for i = 2:lvls % iteratively add random effects noise
        ...
        if ischar(covPat{i-1})
            switch covPat{i - 1}
                ...
                otherwise
                    error(['Did not understand covPat{' int2str(i - 1) '}: ' covPat{i} '. Options are full, diagonal, isotropic, or custom random effects covariance matrix']);
            end
        else
            if ismatrix(covPat{i-1})
                tmp_wRE = covPat{i-1};
            else
                error(['Did not understand covPat{' int2str(i - 1) '}. Options are full, diagonal, isotropic, or custom random effects covariance matrix']);
            end
        end
        ...
        ll_blk = cell(n_units,1);
        ii = 0;
        jj = 0;
        re = mvnrnd(zeros(1,n_re_betas),wRE{i},n_units)';
        re = re(:);
        for j = 1:n_units
            this_grp_idx = find(uniq_grp_elem(j) == this_grp);
            ll_blk{j} = ll{i}(this_grp_idx,:);
            ii = ii + size(ll_blk{j},1);
            jj = jj + size(ll_blk{j},2);
        end
        if jj ~= n_re_betas*n_units || ii ~= n
        ...
        if sparsity < sparsityLim
            Z = spalloc(ii,jj,nz); % preallocate sparse matrix
        else
            Z = zeros(ii,jj);
        end
        last_ii = 0;
