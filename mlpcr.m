...
function [weights, Intercept, lme, CM, SCW] = mlpcr(X,Y,varargin)
    ...
    for i = 1:length(SCW)
        lambda_cnt(i) = sum(var(SCW{i}) > 10^-7);
    end
    if fitintercept{1}
        df = size(X,1) - 1;
    else
    ...
    if df < sum(lambda_cnt) + size(cell2mat(covs),2) + 1
        ...
        warning('MLPCR:dimOverflow','Dropping %d dims from bottom most level.',del);
    ...
    % TODO: move this to mlpca code where it more logicaly belongs
    % TODO: add a printout of the amount of variance transfered from between
    %       to within
