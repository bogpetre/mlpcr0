...
function [pred, STATS] = mlpcr_cv_pred(C,X,Y,varargin)
    ...
    mlpcrOpts = {'dependentJob','fitlmeoptions','mergeWeights'};
    n_d = {};
    dLabels = {};
    id = {};
    vcomps = {};
    ...
    parfor i = 1:kfolds
        ...
        mergeWeights = false;
        if any(strcmp(arg_train,'mergeWeights'))
            mergeWeights = arg_train{find(strcmp(arg_train,'mergeWeights')) + 1};
        end
        if ~mergeWeights
            % Computing final weight map is time consuming and unnecessary
            % since we have component level observations in training sets,
            % hence {'mergeWeights',false} if no mergeWeights option
            % specified. However this isn't ideal, because weight merger 
            % has a nonunique solution and introduces some error, which 
            % here we're not accounting for if we're using non-merged 
            % weights. We leave {'mergeWeights',false} as the default for
            % compatibility with some legacy code, but this should be
            % updated so that mergeWeights true is the default in future
            % versions once this backwards compatibility is no longer 
            % needed.
            [w, offset] = mlpcr(X_train,Y_train,arg_train{:},'mergeWeights',false);

            Y_test = Y(test_idx);
            pred{i} = zeros(length(Y_test),1);
            for j = 1:length(vcomps)
                test_vcomp = vcomps{j}(test_idx,:) - repmat(mean(vcomps{j}(train_idx,:)),length(test_idx),1);
                pred{i} = pred{i} + test_vcomp*w{j} + offset{j};
            end
            pred{i} = pred{i} + mean(Y_train);
        else
